import './EmployerMyListingsEditListingExpanded.css'

export default function EmployerMyListingsEditListingExpanded() {
  return (
    <div className="employer-my-listings-edit-listing-expanded">
      <div className="browser-bar">
        <div className="container-1">
          <div className="action-bttn">
            <div className="close-btn">
            </div>
            <div className="restore-btn">
            </div>
            <div className="minimize-btn">
            </div>
          </div>
          <div className="prev-btn">
            <img className="arrow" src="assets/vectors/Unknown" />
          </div>
          <div className="prev-btn-2">
            <img className="arrow-1" src="assets/vectors/Unknown" />
          </div>
          <img className="option-btn" src="assets/vectors/Unknown" />
        </div>
        <div className="address-bar">
          <div className="safaribrowser">
          safaribrowser
          </div>
          <img className="refresh" src="assets/vectors/Unknown" />
        </div>
        <div className="container">
          <div className="share-btn">
            <img className="share" src="assets/vectors/Unknown" />
          </div>
          <div className="view-search-btn">
            <img className="view" src="assets/vectors/Unknown" />
          </div>
          <img className="add-btn" src="assets/vectors/Unknown" />
        </div>
      </div>
      <div className="container-2">
        <div className="sidebar">
          <div className="sidebar-free-component">
            <div className="job-ez-word-mark">
              <span className="job-ez">
              JobEZ
              </span>
            </div>
            <div className="frame-1000004349">
              <div className="frame-1000004346">
                <div className="list">
                  <img className="vector-7" src="assets/vectors/Unknown" />
                </div>
              </div>
              <div className="frame-1000004347">
                <div className="list-1">
                  <div className="list-item">
                    <img className="vector-8" src="assets/vectors/Unknown" />
                  </div>
                  <div className="list-item-1">
                    <img className="vector-9" src="assets/vectors/Unknown" />
                  </div>
                  <div className="list-item-2">
                    <img className="vector-10" src="assets/vectors/Unknown" />
                  </div>
                </div>
              </div>
              <div className="frame-1000004348">
                <div className="list-2">
                  <div className="list-item-3">
                    <img className="vector-11" src="assets/vectors/Unknown" />
                  </div>
                  <div className="list-item-4">
                    <img className="vector-12" src="assets/vectors/Unknown" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="frame-1000004358">
          <div className="navbar">
            <div className="logo">
              <span className="review-listing">
              Review Listing
              </span>
            </div>
            <div className="button">
              <span className="text">
              Delete
              </span>
              <div className="trash-outline">
                <img className="vector-2" src="assets/vectors/Unknown" />
              </div>
            </div>
          </div>
          <div className="frame-1029">
            <div className="listing-card-web">
              <div className="heading">
                <div className="group-1028">
                  <div className="main-info">
                    <div className="avatar">
                      <span className="ph">
                      PH
                      </span>
                    </div>
                    <div className="heading-text">
                      <div className="heading-1">
                      Barista/Cashier
                      </div>
                      <span className="text-1">
                      Backroom Coffee Shop
                      </span>
                    </div>
                  </div>
                  <div className="job-tags">
                    <div className="badge">
                      <div className="bell-on">
                        <img className="vector-3" src="assets/vectors/Unknown" />
                      </div>
                      <span className="text-5">
                      Urgently Hiring
                      </span>
                    </div>
                    <div className="badge-1">
                      <div className="globe">
                        <img className="vector-4" src="assets/vectors/Unknown" />
                      </div>
                      <span className="text-6">
                      Offers Sponsorship
                      </span>
                    </div>
                  </div>
                </div>
                <div className="buttons">
                  <div className="button-1">
                    <span className="text-7">
                    Edit Listing
                    </span>
                  </div>
                  <div className="indicator-examples">
                    <div className="button-2">
                      <span className="text-8">
                      Minimize View
                      </span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="listing-description">
                <div className="heading-2">
                Listing Description
                </div>
                <span className="text-2">
                We&#39;re in search of a versatile barista to join our team at the café. This role requires proficiency in all aspects of coffee making, including brewing various espresso drinks to perfection and ensuring consistent quality. Additionally, you&#39;ll handle a variety of pastries, maintaining their presentation and freshness. Operating the register efficiently is crucial, so experience with cash handling and transactions is preferred. Our environment is fast-paced, demanding strong multitasking abilities and exceptional customer service skills.
                </span>
              </div>
              <div className="frame-1015">
                <div className="pay-info">
                  <div className="input-number">
                    <div className="label-1">
                    Pay Rate
                    </div>
                    <div className="input-3">
                      <span className="container">
                      $18.50/hour
                      </span>
                    </div>
                  </div>
                  <div className="input-number-1">
                    <div className="label-2">
                    Payment Method
                    </div>
                    <div className="input-4">
                      <span className="container-1">
                      Check
                      </span>
                    </div>
                  </div>
                </div>
                <div className="preferred-timing">
                  <div className="input-number-2">
                    <div className="label-3">
                    Open Hours
                    </div>
                    <div className="input-5">
                      <span className="container-2">
                      9am-5pm
                      </span>
                    </div>
                  </div>
                  <div className="frame-1000004350">
                    <div className="open-days">
                    Open Days
                    </div>
                    <div className="datepicker">
                      <div className="calendar-month">
                        <img className="vector-5" src="assets/vectors/Unknown" />
                      </div>
                      <span className="select-period">
                      Select period
                      </span>
                    </div>
                  </div>
                </div>
                <div className="extra-info">
                  <div className="input-number-3">
                    <div className="label-4">
                    Location
                    </div>
                    <div className="input-6">
                      <div className="map-pin">
                        <img className="vector-6" src="assets/vectors/Unknown" />
                      </div>
                      <span className="container-3">
                      New York, New York
                      </span>
                    </div>
                  </div>
                  <div className="input-number-4">
                    <div className="label-5">
                    Industry
                    </div>
                    <div className="input-7">
                      <span className="container-4">
                      Food Service
                      </span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="listing-description-1">
                <div className="heading-3">
                Hard Requirements
                </div>
                <div className="frame-1000004353">
                  <span className="text-3">
                  Req 1<br />
                  Req 2<br />
                  Req 3<br />
                  Req 4
                  </span>
                  <span className="text-4">
                  Req 1<br />
                  Req 2<br />
                  Req 3<br />
                  Req 4
                  </span>
                </div>
              </div>
              <div className="textarea">
                <div className="label">
                Custom Questions
                </div>
                <div className="input">
                  <span className="question-1">
                  Question 1
                  </span>
                </div>
                <div className="input-1">
                  <span className="question-2">
                  Question 2
                  </span>
                </div>
                <div className="input-2">
                  <span className="question-3">
                  Question 3
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}