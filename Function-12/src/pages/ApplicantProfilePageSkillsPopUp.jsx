import './ApplicantProfilePageSkillsPopUp.css'

export default function ApplicantProfilePageSkillsPopUp() {
  return (
    <div className="applicant-profile-page-skills-pop-up">
      <div className="container-3">
        <div className="browser-bar">
          <div className="container-7">
            <div className="action-bttn">
              <div className="close-btn">
              </div>
              <div className="restore-btn">
              </div>
              <div className="minimize-btn">
              </div>
            </div>
            <div className="prev-btn">
              <img className="arrow" src="assets/vectors/Unknown" />
            </div>
            <div className="prev-btn-1">
              <img className="arrow-1" src="assets/vectors/Unknown" />
            </div>
            <img className="option-btn" src="assets/vectors/Unknown" />
          </div>
          <div className="address-bar">
            <div className="safaribrowser">
            safaribrowser
            </div>
            <img className="refresh" src="assets/vectors/Unknown" />
          </div>
          <div className="container-5">
            <div className="share-btn">
              <img className="share" src="assets/vectors/Unknown" />
            </div>
            <div className="view-search-btn">
              <img className="view" src="assets/vectors/Unknown" />
            </div>
            <img className="add-btn" src="assets/vectors/Unknown" />
          </div>
        </div>
        <div className="frame-1000004341">
          <div className="bar">
            <span className="text-12">
            Applied to: Listing Title
            </span>
          </div>
          <div className="content">
            <div className="container">
              <div className="header">
                <div className="applicant-deets">
                  <div className="avatar">
                  </div>
                  <div className="container-2">
                    <p className="joey-smith-he-him">
                    <span className="joey-smith-he-him-sub-0"></span><span></span>
                    </p>
                    <div className="container-4">
                      <div className="badge-11">
                        <span className="text-11">
                        International
                        </span>
                      </div>
                      <div className="badge-10">
                        <span className="text-10">
                        Referred
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="button">
                  <img className="folder-open" src="assets/vectors/Unknown" />
                  <span className="text-21">
                  View Files
                  </span>
                </div>
              </div>
              <div className="about">
                <div className="about-applicant">
                About Applicant Name:
                </div>
                <span className="about-body">
                Hello! I&#39;m Joey, a passionate barista with over three years of experience. I excel in creating exceptional coffee experiences through my expertise in brewing, dedication to cleanliness, and commitment to outstanding customer service. With a warm smile and a genuine love for coffee, I strive to make each interaction memorable and enjoyable for every customer. I&#39;m excited about the opportunity to bring my skills to your team and contribute to the world of specialty coffee.
                </span>
              </div>
              <div className="frame-1015">
                <div className="applicant-application">
                  <div className="input-number">
                    <div className="label">
                    Employment Status
                    </div>
                    <div className="input">
                      <span className="container">
                      Employed
                      </span>
                    </div>
                  </div>
                  <div className="input-number-1">
                    <div className="label-1">
                    Application Status
                    </div>
                    <div className="input-1">
                      <span className="container-1">
                      Unreviewed
                      </span>
                    </div>
                  </div>
                </div>
                <div className="preferred-timing">
                  <div className="input-number-2">
                    <div className="label-2">
                    Preferred Hours
                    </div>
                    <div className="input-2">
                      <span className="container-2">
                      9am-5pm
                      </span>
                    </div>
                  </div>
                  <div className="frame-1000004350">
                    <div className="preferred-days">
                    Preferred Days
                    </div>
                    <div className="datepicker">
                      <div className="calendar-month">
                        <img className="vector-11" src="assets/vectors/Unknown" />
                      </div>
                      <span className="select-period">
                      Select period
                      </span>
                    </div>
                  </div>
                </div>
                <div className="preferred-timing-1">
                  <div className="input-number-3">
                    <div className="label-3">
                    Location
                    </div>
                    <div className="input-3">
                      <div className="map-pin">
                        <img className="vector-12" src="assets/vectors/Unknown" />
                      </div>
                      <span className="container-3">
                      New York, New York
                      </span>
                    </div>
                  </div>
                  <div className="input-number-4">
                    <div className="label-4">
                    Spoken Languages
                    </div>
                    <div className="input-4">
                      <span className="container-4">
                      English, Spanish
                      </span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="experience-and-skills">
                <div className="experience">
                  <p className="relevant-experience">
                  <span className="relevant-experience-sub-37"></span><span className="relevant-experience-sub-55"></span><span></span>
                  </p>
                  <div className="list">
                    <div className="list-item">
                      <span className="suporting-text">
                      Starbucks Barista
                      </span>
                      <div className="expand-outline">
                        <img className="vector-13" src="assets/vectors/Unknown" />
                      </div>
                    </div>
                    <div className="list-item-1">
                      <div className="border-t">
                      </div>
                      <div className="content-1">
                        <span className="suporting-text-1">
                        Babysitter
                        </span>
                        <div className="expand-outline-1">
                          <img className="vector-14" src="assets/vectors/Unknown" />
                        </div>
                      </div>
                    </div>
                    <div className="list-item-2">
                      <div className="border-t-1">
                      </div>
                      <div className="content-2">
                        <span className="suporting-text-2">
                        Applebee’s Waiter
                        </span>
                        <div className="expand-outline-2">
                          <img className="vector-15" src="assets/vectors/Unknown" />
                        </div>
                      </div>
                    </div>
                    <div className="list-item-3">
                      <div className="border-t-2">
                      </div>
                      <div className="content-3">
                        <span className="suporting-text-3">
                        Starbucks Cashier
                        </span>
                        <div className="expand-outline-3">
                          <img className="vector-16" src="assets/vectors/Unknown" />
                        </div>
                      </div>
                    </div>
                    <div className="list-item-4">
                      <div className="border-t-3">
                      </div>
                      <div className="content-4">
                        <span className="suporting-text-4">
                        Camp Counselor
                        </span>
                        <div className="expand-outline-4">
                          <img className="vector-17" src="assets/vectors/Unknown" />
                        </div>
                      </div>
                    </div>
                    <div className="list-item-5">
                      <div className="border-t-4">
                      </div>
                      <div className="content-5">
                        <span className="suporting-text-5">
                        See all
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="skills-1">
                  <div className="skills-2">
                  Skills:
                  </div>
                  <div className="frame-10000043242">
                    <div className="frame-10000043251">
                      <div className="badge-14">
                        <span className="text-15">
                        Teamwork
                        </span>
                      </div>
                    </div>
                    <div className="frame-10000043261">
                      <div className="badge-15">
                        <span className="text-16">
                        Serving
                        </span>
                      </div>
                    </div>
                    <div className="frame-10000043271">
                      <div className="badge-16">
                        <span className="text-17">
                        Customer Service
                        </span>
                      </div>
                    </div>
                    <div className="badge-17">
                      <span className="text-18">
                      Effeciency
                      </span>
                    </div>
                    <div className="badge-18">
                      <span className="text-19">
                      Coffee Making
                      </span>
                    </div>
                    <div className="button-1">
                      <span className="text-20">
                      See all
                      </span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="custom">
                <p className="relevant-experience-1">
                <span className="relevant-experience-1-sub-0"></span><span className="relevant-experience-1-sub-40"></span><span className="relevant-experience-1-sub-39"></span><span className="relevant-experience-1-sub-40"></span><span className="relevant-experience-1-sub-39"></span><span className="relevant-experience-1-sub-40"></span><span className="relevant-experience-1-sub-39"></span><span className="relevant-experience-1-sub-40"></span><span></span>
                </p>
              </div>
            </div>
            <div className="button-2">
              <span className="text-22">
              View Contact Info
              </span>
            </div>
          </div>
        </div>
      </div>
      <div className="container-1">
        <div className="sidebar">
          <div className="sidebar-free-component">
            <div className="job-ez-word-mark">
              <span className="job-ez">
              JobEZ
              </span>
            </div>
            <div className="frame-1000004349">
              <div className="frame-1000004346">
                <div className="list-1">
                  <img className="vector-3" src="assets/vectors/Unknown" />
                </div>
              </div>
              <div className="frame-1000004347">
                <div className="list-2">
                  <div className="list-item-6">
                    <img className="vector-4" src="assets/vectors/Unknown" />
                  </div>
                  <div className="list-item-7">
                    <img className="vector-5" src="assets/vectors/Unknown" />
                  </div>
                  <div className="list-item-8">
                    <img className="vector-6" src="assets/vectors/Unknown" />
                  </div>
                </div>
              </div>
              <div className="frame-1000004348">
                <div className="list-3">
                  <div className="list-item-9">
                    <img className="vector-7" src="assets/vectors/Unknown" />
                  </div>
                  <div className="list-item-10">
                    <img className="vector-8" src="assets/vectors/Unknown" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="modal">
          <div className="container-6">
            <div className="modal-header">
              <div className="title">
                <span className="skills">
                Skills
                </span>
              </div>
              <div className="x">
                <img className="vector-2" src="assets/vectors/Unknown" />
              </div>
            </div>
            <div className="separator">
            </div>
          </div>
          <div className="modal-body">
            <div className="paragraph-1">
              <div className="next-friday-should-b">
              Relevant Skills:
              </div>
              <div className="frame-1000004324">
                <div className="frame-1000004325">
                  <div className="badge">
                    <span className="text">
                    Teamwork
                    </span>
                  </div>
                  <div className="badge-1">
                    <span className="text-1">
                    Serving
                    </span>
                  </div>
                  <div className="badge-2">
                    <span className="text-2">
                    Efficiency
                    </span>
                  </div>
                </div>
                <div className="frame-1000004326">
                  <div className="badge-3">
                    <span className="text-3">
                    Customer Service
                    </span>
                  </div>
                  <div className="badge-4">
                    <span className="text-4">
                    Coffee Making
                    </span>
                  </div>
                </div>
              </div>
            </div>
            <div className="paragraph-2">
              <div className="all-skills">
              All Skills:
              </div>
              <div className="frame-10000043241">
                <div className="frame-1000004327">
                  <div className="badge-5">
                    <span className="text-5">
                    Childcare
                    </span>
                  </div>
                  <div className="badge-6">
                    <span className="text-6">
                    Time Management
                    </span>
                  </div>
                </div>
                <div className="frame-1000004328">
                  <div className="badge-7">
                    <span className="text-7">
                    Honest
                    </span>
                  </div>
                  <div className="badge-8">
                    <span className="text-8">
                    Stamina
                    </span>
                  </div>
                  <div className="badge-9">
                    <span className="text-9">
                    Patient
                    </span>
                  </div>
                </div>
                <div className="frame-1000004329">
                  <div className="badge-12">
                    <span className="text-13">
                    Multitasking
                    </span>
                  </div>
                  <div className="badge-13">
                    <span className="text-14">
                    Communication
                    </span>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}