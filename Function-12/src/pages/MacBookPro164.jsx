import './MacBookPro164.css'

export default function MacBookPro164() {
  return (
    <div className="mac-book-pro-164">
      <div className="group-333">
        <div className="container-13">
          <div className="action-bttn">
            <div className="close-btn">
            </div>
            <div className="restore-btn">
            </div>
            <div className="minimize-btn">
            </div>
          </div>
          <div className="prev-btn">
            <img className="arrow" src="assets/vectors/Unknown" />
          </div>
          <div className="prev-btn-2">
            <img className="arrow-1" src="assets/vectors/Unknown" />
          </div>
          <img className="option-btn" src="assets/vectors/Unknown" />
        </div>
        <div className="address-bar">
          <div className="safaribrowser">
          safaribrowser
          </div>
          <img className="refresh" src="assets/vectors/Unknown" />
        </div>
        <div className="container-9">
          <div className="share-btn">
            <img className="share" src="assets/vectors/Unknown" />
          </div>
          <div className="view-search-btn">
            <img className="view" src="assets/vectors/Unknown" />
          </div>
          <img className="add-btn" src="assets/vectors/Unknown" />
        </div>
      </div>
      <div className="container-6">
        <div className="container">
          <div className="bars-outline">
            <img className="vector-27" src="assets/vectors/Unknown" />
          </div>
          <div className="navbar">
            <span className="flowbite">
            JobEZ
            </span>
          </div>
        </div>
        <div className="container-12">
          <div className="autocomplete-input">
            <div className="content">
              <div className="search-outline">
                <img className="vector-2" src="assets/vectors/Unknown" />
              </div>
              <span className="quick-search-for-anything">
              What part-time job are you seeking?
              </span>
            </div>
            <div className="autocomplete-input-1">
              <div className="search-outline-1">
                <img className="vector-3" src="assets/vectors/Unknown" />
              </div>
              <span className="quick-search-for-anything-1">
              What part-time job are you seeking?
              </span>
            </div>
          </div>
          <div className="button-8">
            <span className="text-17">
            Select Language
            </span>
            <div className="chevron-down-5">
              <img className="vector-28" src="assets/vectors/Unknown" />
            </div>
          </div>
          <div className="logo">
            <span className="flowbite-1">
            SIGN-UP / LOGIN
            </span>
          </div>
        </div>
      </div>
      <div className="container-11">
        <div className="dropdown-buttons">
          <div className="button-10">
            <span className="text-12">
            Area
            </span>
            <div className="chevron-down">
              <img className="vector-22" src="assets/vectors/Unknown" />
            </div>
          </div>
        </div>
        <div className="dropdown-buttons-1">
          <div className="button-11">
            <span className="text-13">
            Recency
            </span>
            <div className="chevron-down-1">
              <img className="vector-23" src="assets/vectors/Unknown" />
            </div>
          </div>
        </div>
        <div className="dropdown-buttons-2">
          <div className="button-12">
            <span className="text-14">
            Pay Rate
            </span>
            <div className="chevron-down-2">
              <img className="vector-24" src="assets/vectors/Unknown" />
            </div>
          </div>
        </div>
        <div className="button-6">
          <span className="text-15">
          Industries
          </span>
          <div className="chevron-down-3">
            <img className="vector-25" src="assets/vectors/Unknown" />
          </div>
        </div>
        <div className="button-7">
          <span className="text-16">
          Shifts
          </span>
          <div className="chevron-down-4">
            <img className="vector-26" src="assets/vectors/Unknown" />
          </div>
        </div>
        <div className="button-9">
          <span className="text-18">
          Hour
          </span>
          <div className="chevron-down-6">
            <img className="vector-29" src="assets/vectors/Unknown" />
          </div>
        </div>
      </div>
      <div className="container-5">
        <div className="container-2">
          <div className="logo-1">
            <span className="flowbite-2">
            135 Results
            </span>
          </div>
          <div className="card">
            <div className="chevron-left-outline">
              <img className="vector-5" src="assets/vectors/Unknown" />
            </div>
            <div className="container-10">
              <div className="skeleton">
                <div className="image">
                  <img className="icon" src="assets/vectors/Unknown" />
                </div>
                <div className="body">
                  <div className="heading">
                    <div className="shape">
                    </div>
                  </div>
                  <div className="text">
                    <div className="text-block">
                    </div>
                    <div className="text-block-1">
                    </div>
                    <div className="text-block-2">
                    </div>
                  </div>
                </div>
                <div className="author">
                  <div className="user-circle">
                    <img className="vector-4" src="assets/vectors/Unknown" />
                  </div>
                  <div className="name-role">
                    <div className="text-block-3">
                      <div className="shape-1">
                      </div>
                    </div>
                    <div className="text-block-4">
                      <div className="shape-2">
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="chevron-right-outline">
                <img className="vector-6" src="assets/vectors/Unknown" />
              </div>
              <div className="button">
                <span className="text-4">
                Apply
                </span>
              </div>
            </div>
          </div>
          <div className="card-1">
            <div className="chevron-left-outline-1">
              <img className="vector-8" src="assets/vectors/Unknown" />
            </div>
            <div className="container-1">
              <div className="skeleton-1">
                <div className="image-1">
                  <img className="icon-1" src="assets/vectors/Unknown" />
                </div>
                <div className="body-1">
                  <div className="heading-1">
                    <div className="shape-3">
                    </div>
                  </div>
                  <div className="text-1">
                    <div className="text-block-5">
                    </div>
                    <div className="text-block-6">
                    </div>
                    <div className="text-block-7">
                    </div>
                  </div>
                </div>
                <div className="author-1">
                  <div className="user-circle-1">
                    <img className="vector-7" src="assets/vectors/Unknown" />
                  </div>
                  <div className="name-role-1">
                    <div className="text-block-8">
                      <div className="shape-4">
                      </div>
                    </div>
                    <div className="text-block-9">
                      <div className="shape-5">
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="chevron-right-outline-1">
                <img className="vector-9" src="assets/vectors/Unknown" />
              </div>
              <div className="button-3">
                <span className="text-7">
                Apply
                </span>
              </div>
            </div>
          </div>
          <div className="card-4">
            <div className="chevron-left-outline-4">
              <img className="vector-17" src="assets/vectors/Unknown" />
            </div>
            <div className="container-14">
              <div className="skeleton-4">
                <div className="image-4">
                  <img className="icon-4" src="assets/vectors/Unknown" />
                </div>
                <div className="body-4">
                  <div className="heading-4">
                    <div className="shape-12">
                    </div>
                  </div>
                  <div className="text-8">
                    <div className="text-block-20">
                    </div>
                    <div className="text-block-21">
                    </div>
                    <div className="text-block-22">
                    </div>
                  </div>
                </div>
                <div className="author-4">
                  <div className="user-circle-4">
                    <img className="vector-16" src="assets/vectors/Unknown" />
                  </div>
                  <div className="name-role-4">
                    <div className="text-block-23">
                      <div className="shape-13">
                      </div>
                    </div>
                    <div className="text-block-24">
                      <div className="shape-14">
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="chevron-right-outline-4">
                <img className="vector-18" src="assets/vectors/Unknown" />
              </div>
              <div className="button-5">
                <span className="text-11">
                Apply
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className="container-7">
          <div className="card-3">
            <div className="chevron-left-outline-3">
              <img className="vector-14" src="assets/vectors/Unknown" />
            </div>
            <div className="container-4">
              <div className="skeleton-3">
                <div className="image-3">
                  <img className="icon-3" src="assets/vectors/Unknown" />
                </div>
                <div className="body-3">
                  <div className="heading-3">
                    <div className="shape-9">
                    </div>
                  </div>
                  <div className="text-3">
                    <div className="text-block-15">
                    </div>
                    <div className="text-block-16">
                    </div>
                    <div className="text-block-17">
                    </div>
                  </div>
                </div>
                <div className="author-3">
                  <div className="user-circle-3">
                    <img className="vector-13" src="assets/vectors/Unknown" />
                  </div>
                  <div className="name-role-3">
                    <div className="text-block-18">
                      <div className="shape-10">
                      </div>
                    </div>
                    <div className="text-block-19">
                      <div className="shape-11">
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="chevron-right-outline-3">
                <img className="vector-15" src="assets/vectors/Unknown" />
              </div>
              <div className="button-1">
                <span className="text-5">
                Apply
                </span>
              </div>
            </div>
          </div>
          <div className="card-2">
            <div className="chevron-left-outline-2">
              <img className="vector-11" src="assets/vectors/Unknown" />
            </div>
            <div className="container-3">
              <div className="skeleton-2">
                <div className="image-2">
                  <img className="icon-2" src="assets/vectors/Unknown" />
                </div>
                <div className="body-2">
                  <div className="heading-2">
                    <div className="shape-6">
                    </div>
                  </div>
                  <div className="text-2">
                    <div className="text-block-10">
                    </div>
                    <div className="text-block-11">
                    </div>
                    <div className="text-block-12">
                    </div>
                  </div>
                </div>
                <div className="author-2">
                  <div className="user-circle-2">
                    <img className="vector-10" src="assets/vectors/Unknown" />
                  </div>
                  <div className="name-role-2">
                    <div className="text-block-13">
                      <div className="shape-7">
                      </div>
                    </div>
                    <div className="text-block-14">
                      <div className="shape-8">
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="chevron-right-outline-2">
                <img className="vector-12" src="assets/vectors/Unknown" />
              </div>
              <div className="button-2">
                <span className="text-6">
                Apply
                </span>
              </div>
            </div>
          </div>
          <div className="card-5">
            <div className="chevron-left-outline-5">
              <img className="vector-20" src="assets/vectors/Unknown" />
            </div>
            <div className="container-8">
              <div className="skeleton-5">
                <div className="image-5">
                  <img className="icon-5" src="assets/vectors/Unknown" />
                </div>
                <div className="body-5">
                  <div className="heading-5">
                    <div className="shape-15">
                    </div>
                  </div>
                  <div className="text-9">
                    <div className="text-block-25">
                    </div>
                    <div className="text-block-26">
                    </div>
                    <div className="text-block-27">
                    </div>
                  </div>
                </div>
                <div className="author-5">
                  <div className="user-circle-5">
                    <img className="vector-19" src="assets/vectors/Unknown" />
                  </div>
                  <div className="name-role-5">
                    <div className="text-block-28">
                      <div className="shape-16">
                      </div>
                    </div>
                    <div className="text-block-29">
                      <div className="shape-17">
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="chevron-right-outline-5">
                <img className="vector-21" src="assets/vectors/Unknown" />
              </div>
              <div className="button-4">
                <span className="text-10">
                Apply
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className="container">
        </div>
      </div>
    </div>
  )
}